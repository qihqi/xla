...
class SHLOModel:
  ...
  def __init__(self, bundle, default_method_name='forward'):
    ...
    self._name_to_stablehlo = {
      meta.name: (meta, stablehlo) for meta, stablehlo in bundle.stablehlo_funcs
    }
...
def export_torch_model(model: torch.nn.Module,
  ...
  * sufficiently similar input because this function will use tracing to extract the model operations
  ...
  args = tuple(val.to(device=device) if isinstance(val, torch.Tensor) else val for val in sample_inputs)
...
def _callable_to_stablehlo_bundle(func, input_args, state_dict):
  ...
  input_ids = {torch_xla._XLAC._xla_get_tensor_id(tensor): pos
...
def _exported_program_to_stablehlo_bundle(exported_model, args) -> StableHLOModelBundle:
  device = xm.xla_device()
  ...
  state = {name: val.to(device=device) for name, val in exported_model.state_dict.items()
  ...
  bundle.state_dict = { key: val.detach().cpu().numpy() if isinstance(val, torch.Tensor) else val
